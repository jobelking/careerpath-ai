"""
Groq API Service
Handles communication with Groq LLM via the OpenAI Responses API
for generating personalized learning roadmaps and certification recommendations.
"""
...
# OpenAI client pointed at Groq's Responses API
client = OpenAI(
...
def generate_learning_roadmap(career_path: str, resume_text: str) -> Dict[str, Any]:
    ...
    Returns:
        Dict with structure: { "analysis_summary": str, "improvement_areas": [...] }
...
- Use the following URL formats:
  ...
  * Skillshare: https://www.skillshare.com/en/search?query=[Topic]
  * O*NET (Job Details): https://www.onetonline.org/find/result?s=[Topic]
  * CareerOneStop: https://www.careeronestop.org/ExploreCareers/Learn/career-profiles.aspx?keyword=[Topic]
...
- Ensure valid JSON"""
    ...
    try:
        # Call Groq via OpenAI Responses API with json_object format
        response = client.responses.create(
            model="openai/gpt-oss-120b",
            input=[
                {
                    "role": "system",
                    "content": "You are an expert career advisor. Always respond with valid JSON only, no markdown, no extra text."
                },
                {
                    "role": "user",
                    "content": prompt
...
- Ensure valid JSON"""
    ...
    try:
        ...
        # Parse the guaranteed JSON response
        try:
            certifications_data = json.loads(response.output_text)
        except json.JSONDecodeError as e:
            raise Exception(f"Invalid JSON from Groq: {str(e)}. Response: {response.output_text[:300]}")
